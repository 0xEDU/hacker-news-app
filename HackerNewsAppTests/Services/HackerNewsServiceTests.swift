...
final class HackerNewsServiceTests: XCTestCase {
    ...
    func test_fetchTopStories_whenAPIReturnsValidData_shouldPopulateStories() async throws {
        // Arrange
        ...
        // Act
        let stories = try await sut.fetchTopStories()
    ...
    func test_fetchTopStories_whenNetworkFails_shouldThrowError() async {
        // Arrange
        MockURLProtocol.mockResponses["https://hacker-news.firebaseio.com/v0/topstories.json"] =
        ...
        // Act & Assert
        do {
            _ = try await sut.fetchTopStories()
            XCTFail("Should have thrown an error")
        } catch {
            XCTAssertTrue(error is HackerNewsErrorEnum)
        }
    }
